import { useTranslation } from 'react-i18next';
import { useState, type ReactNode } from 'react';

// Components
import { Icon, Tabs } from 'ama-design-system';

export function ObservatoryInfoTabs() {
  const { t } = useTranslation();
  const [activeTab, setActiveTab] = useState('tab1');

  // Each vertical tab
  const tab = (paragraph: string, bullet1: string, bullet2: string, bullet3: string, isActive: boolean): ReactNode => {
    return (
      <div className="tabs_info_container mt-3" tabIndex={isActive ? 0 : -1}>
        <p className="ama-typography-body-large">{paragraph}</p>
        <ul className="ps-0">
          <li className="d-flex p-1">
            <Icon name="AMA-Ponto-Solid" />
            <p className="ama-typography-body-large pt-2">{bullet1}</p>
          </li>
          <li className="d-flex p-1">
            <Icon name="AMA-Ponto-Solid" />
            <p className="ama-typography-body-large pt-2">{bullet2}</p>
          </li>
          <li className="d-flex p-1">
            <Icon name="AMA-Ponto-Solid" />
            <p className="ama-typography-body-large pt-2">{bullet3}</p>
          </li>
        </ul>
      </div>
    );
  };

  // Vertical tabs
  const tabsGoodBad = [
    {
      eventKey: 'tab1',
      title: t('HOME.tabs._1.title'),
      component: tab(t('HOME.tabs._1.paragraph'), t('HOME.tabs._1.bullet1'), t('HOME.tabs._1.bullet2'), t('HOME.tabs._1.bullet3'), activeTab === 'tab1'),
    },
    {
      eventKey: 'tab2',
      title: t('HOME.tabs._2.title'),
      component: tab(t('HOME.tabs._2.paragraph'), t('HOME.tabs._2.bullet1'), t('HOME.tabs._2.bullet2'), t('HOME.tabs._2.bullet3'), activeTab === 'tab2'),
    },
    {
      eventKey: 'tab3',
      title: t('HOME.tabs._3.title'),
      component: tab(t('HOME.tabs._3.paragraph'), t('HOME.tabs._3.bullet1'), t('HOME.tabs._3.bullet2'), t('HOME.tabs._3.bullet3'), activeTab === 'tab3'),
    },
  ];

  return (
    <div className="d-flex justify-content-between">
      <Tabs
        tabs={tabsGoodBad}
        title={t('HOME.tabs.title')}
        defaultActiveKey="tab1"
        vertical={true}
        onSelect={(key) => setActiveTab(key)}
        className="tabs_container"
      />
    </div>
  );
}
