...
export class ObservatoryNumbersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const totalDeclarationsWeb =
      this.declarations.total.websites.conform +
      this.declarations.total.websites.partial +
      this.declarations.total.websites.not_conform;

    const totalDeclarationsApps =
      this.declarations.total.apps.conform +
      this.declarations.total.apps.partial +
      this.declarations.total.apps.not_conform + 1;//FIXME

    const declarationBars = document.querySelectorAll(".declaration .bar");

    this.fillBar(<HTMLElement>declarationBars[0], "#15ac51",
      ((this.declarations.total.websites.conform -
    ...
    this.fillBar(<HTMLElement>declarationBars[2], "#f3d609",
      ((this.declarations.total.websites.partial -
    ...
    this.fillBar(<HTMLElement>declarationBars[4], "#e90018",
      ((this.declarations.total.websites.not_conform -
    ...
    this.fillBar(<HTMLElement>declarationBars[6], "#15ac51",
      ((this.declarations.total.websites.conform - this.declarations.currentYear.websites.conform) / totalDeclarationsWeb) * 100
    );
    this.fillBar(
      <HTMLElement>declarationBars[7],
      "#15ac51",
      (this.declarations.currentYear.websites.conform / totalDeclarationsWeb) * 100
    );
    this.fillBar(<HTMLElement>declarationBars[8], "#f3d609", ((this.declarations.total.websites.partial - this.declarations.currentYear.websites.partial) / totalDeclarationsWeb) * 100);
    this.fillBar(
      <HTMLElement>declarationBars[9],
      "#f3d609",
      (this.declarations.currentYear.websites.partial / totalDeclarationsWeb) * 100
    );
    this.fillBar(<HTMLElement>declarationBars[10], "#e90018", ((this.declarations.total.websites.not_conform - this.declarations.currentYear.websites.not_conform) / totalDeclarationsWeb) * 100);
    this.fillBar(
      <HTMLElement>declarationBars[11],
      "#e90018",
      (this.declarations.currentYear.websites.not_conform / totalDeclarationsWeb) *
      100
    );

    this.fillBar(<HTMLElement>declarationBars[12], "#15ac51", ((this.declarations.total.apps.conform - this.declarations.currentYear.apps.conform) / totalDeclarationsApps) * 100);
    this.fillBar(
      <HTMLElement>declarationBars[13],
      "#15ac51",
      (this.declarations.currentYear.apps.conform / totalDeclarationsApps) * 100
    );
    this.fillBar(<HTMLElement>declarationBars[14], "#f3d609", ((this.declarations.total.apps.partial - this.declarations.currentYear.apps.partial) / totalDeclarationsApps) * 100);
    this.fillBar(
      <HTMLElement>declarationBars[15],
      "#f3d609",
      (this.declarations.currentYear.apps.partial / totalDeclarationsApps) * 100
    );
    this.fillBar(
      <HTMLElement>declarationBars[16],
      "#e90018",

      ((this.declarations.total.apps.not_conform - this.declarations.currentYear.apps.not_conform) / totalDeclarationsApps) * 100
    );
    this.fillBar(
      <HTMLElement>declarationBars[17],
      "#e90018",
      (this.declarations.currentYear.apps.not_conform / totalDeclarationsApps) * 100
    );
  ...
  private fillBar(
    ...
    element.style.background = `-webkit-linear-gradient(left, ${color}, ${color} ${percentage}%, #dcdcdb ${percentage}%, #dcdcdb ${100 - percentage
      }%)`;

    element.style.background = `-moz-linear-gradient(left, ${color}, ${color} ${percentage}%, #dcdcdb ${percentage}%, #dcdcdb ${100 - percentage
      }%)`;

    element.style.background = `-ms-linear-gradient(left, ${color}, ${color} ${percentage}%, #dcdcdb ${percentage}%, #dcdcdb ${100 - percentage
      }%)`;

    element.style.background = `-linear-gradient(left, ${color}, ${color} ${percentage}%, #dcdcdb ${percentage}%, #dcdcdb ${100 - percentage
      }%)`;
  }
}
